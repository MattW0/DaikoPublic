---
tags:
  - Admin
thumbnail:
concerning: History
context: Party
---
*Played on the 09.01.2026*

## 1st Approach
1) At wizards tower (Ahmed Noke impostering as the true master)
2) Convince the apes to get invited by Ahmed
3) Make him taste and convince him to show up to the opening on Friday
4) Heir leaves an Eerie Token to observe the wizards initial reaction: He commands the ape to give him the wine barrel
5) Party retreats to chill spot and meet [[Nations/Daicon Kingdom/Towns/Dunmarrow/NPCs/Vulgrim Savage|Vulgrim Savage]] again
6) Smithing intricate plan to infiltrate (is killing the monkeys okay, Heir in depth agexplains verbal component of guidance)

## 2nd Approach
1) Lizzy sneaks in but attracts attention
2) Fight breaks out with lizzys baboon familiar
3) Art & Heir fire from woods.
4) Wizzy sneaks in and checks levers, Heir goes down
5) Wizzy half breaks wand, Art charges through bookshelf trying to interrupt dark ritual
6) Bed-table-dragon summoning with enlarge spell
7) Concentration broken to collapse of dragon
8) Lizzy sees gun of Colt and shoots ahmed

```encounter-table
name: Shizzard wizzard tower
creatures:
- 2: Ape
- 1: Gorilla with Sword
- 1: Brown Bear
- 1: Lizzy Familiar
```
## Aftering
Martha and Andrew: Parents of Kevin
- Kevin,16, goes to bed and so do Art and Lizzy
	- Parents martha and Andrew
- Heir goes to bed with [[Nations/Daicon Kingdom/Towns/Dunmarrow/NPCs/Vulgrim Savage|Vulgrim,]]

_Last edited: 17.04.2026, 00:23_
